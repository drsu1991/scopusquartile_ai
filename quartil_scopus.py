...
# ============================
if query:
    # Cek apakah kolom Singkatan ada
    if "Singkatan" not in df.columns:
        st.error("Dataset tidak memiliki kolom 'Singkatan'. Harap periksa CSV Anda.")
    else:
        # Cari exact match singkatan
        exact_match = df[df["Singkatan"].str.lower() == query.lower()]

        if not exact_match.empty:
            jurnal = exact_match.iloc[0]
            st.success("✅ Ditemukan berdasarkan singkatan")

        else:
            # Jika tidak ada singkatan persis, cari nama jurnal
            matches = df[
                df["Nama Jurnal"].str.contains(query, case=False, na=False)
            ]

            if not matches.empty:
                jurnal = matches.iloc[0]
                st.success("✅ Ditemukan berdasarkan nama jurnal")
            else:
                st.warning("⚠️ Jurnal tidak ditemukan.")
                jurnal = None

        if 'jurnal' in locals() and jurnal is not None:
...
st.markdown("---")
st.caption(
    "Developed by dr. Suhendra Mandala Ernas | "
